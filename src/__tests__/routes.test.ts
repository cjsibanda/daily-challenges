...
describe('routes', () => {
  ...
  describe('GET /api/daily-challenge/date/:date', () => {
    it('should return 400 for an invalid date format', async () => {
      const response = await request(app).get('/api/daily-challenge/date/invalid-format');
      expect(response.status).toBe(400);
      expect(response.body).toHaveProperty('error');
      expect(response.body.error).toEqual('Invalid date format: "invalid-format". Please use "M-D-YYYY"');
    });

    it('should return 400 for an invalid calendar date', async () => {
      const response = await request(app).get('/api/daily-challenge/date/99-99-9999');
      expect(response.status).toBe(400);
      expect(response.body).toHaveProperty('error');
